...
- $GM_{ON} \cdot \left(\frac{\sum_{j=0}^{N_{sensors}} BL_{j}}{N_{sensors}} - \delta\right)$, donde $\delta = (\max\{RL_{i}\} - 0.5)$ si $\max\{RL_{i}\} > 0.93$, y $\delta = 0$ en caso contrario. Evalúa que en el caso de haber "agarrado un objeto" (pasar por una baldosa gris), el robot se dirija hacia la luz azul y no se acerque excesivamente a la luz roja
